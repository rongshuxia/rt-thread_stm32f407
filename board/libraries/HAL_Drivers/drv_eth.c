...
rt_err_t rt_stm32_eth_tx(rt_device_t dev, struct pbuf *p)
{
    ...
    for (q = p; q != NULL; q = q->next)
    {
        ...
        while ((byteslefttocopy + bufferoffset) > ETH_TX_BUF_SIZE)
        {
            ...
            if ((DmaTxDesc->Status & ETH_DMATXDESC_OWN) != (uint32_t)RESET)
            {
                rt_kprintf("dma tx desc buffer is not valid");
                ret = ERR_USE;
                goto error;
            }
    ...
    if (state != HAL_OK)
    {
        rt_kprintf("eth transmit frame faild: %d", state);
    }
...
static void phy_monitor_thread_entry(void *parameter)
{
	...
	while (1)
    {
	    ...
	    HAL_ETH_ReadPHYRegister(&EthHandle, PHY_BASIC_STATUS_REG, (uint32_t *)&status);
        //rt_kprintf("PHY Basic Status Reg: 0x%04X\n", status);
        ...
        if (status & (PHY_AUTONEGO_COMPLETE_MASK | PHY_LINKED_STATUS_MASK))
        {
            ...
            HAL_ETH_ReadPHYRegister(&EthHandle, PHY_BASIC_CONTROL_REG, (uint32_t *)&bcr);
            //LOG_D("PHY Basic Control Reg: 0x%04X \n", bcr);
